package cn.syn.tiles.springmvc.aop;
...
import static cn.syn.tiles.springmvc.utils.DBTools.longToDate;
...
public class DBAop {
    ...
    @Before("execution(* cn.syn.tiles.springmvc.dao.StudentMapper.*(..))")
    public void beginTransaction() {
        time1 = System.currentTimeMillis();
        System.out.println("[前置通知]  开启事务.."+longToDate(time1,"yyyy年-MM月dd日-HH时mm分ss秒"));
    }

    @After("execution(* cn.syn.tiles.springmvc.dao.StudentMapper.*(..))")
...
//        System.out.println(joinPoint.getSignature().getName());
    ...
    @AfterReturning("execution(* cn.syn.tiles.springmvc.dao.StudentMapper.*(..))")
    ...
    @AfterThrowing("execution(* cn.syn.tiles.springmvc.dao.StudentMapper.*(..))")
    public void afterThrowing() {
        System.out.println("[异常通知]");
    }

    @Around(("execution(* cn.syn.tiles.springmvc.dao.StudentMapper.*(..))"))
